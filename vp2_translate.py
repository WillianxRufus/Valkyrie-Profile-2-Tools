...
"""GUI and command-line launcher for the public VP2 translation tools."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    generate.add_argument("images", nargs="*", metavar="IMAGE",
                          help="disc image(s): the USA release, and optionally "
                               "the Japanese one for its original script")
...
def run_gui() -> int:
    if TK_IMPORT_ERROR is not None:
        print(f"this build has no Tk, so the window cannot open: {TK_IMPORT_ERROR}",
              file=sys.stderr)
        return 3
...
def _run_internal_build(arguments) -> int:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv[:1] == ["_runtime-build"]:
        ...
        return _run_internal_build(argv[1:])
    parser = _parser()
    args = parser.parse_args(argv)
    if args.self_check or args.command:
        attach_console_for_output()
    if args.self_check:
